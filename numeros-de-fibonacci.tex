...
Uma solução simples é baseada na indexação de um arranjo (\inh{fibs n
  ! n}), onde a definição de \inh{fibs}, mostrada na seção
\ref{versao-funcional-de-matrixMult}, é apresentada novamente a
seguir:

\begin{center}
\begin{tabular}{l}
\begin{hask}{fatorial,fibs,fib1}
...
(\inh{fibArr!(i-2)} e \inh{fibArr!(i-1)}) e uma soma
(\inh{fibArr!(i-2) + fibArr!(i-1)}), e:
...
Como mostrado na subseção anterior, podemos usar a fórmula:
  ...
  \[ F(\inh{n}) = \frac{1}{\sqrt{5}} (\phi^n + \frac{1}{\phi}^n) \]
onde $\phi = \frac{1 + \sqrt{5}}{2}$ para obter uma complexidade de
