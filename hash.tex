...
-- Insere (chave,val) na tabela de dispersão. Entradas podem ter a
-- mesma chave.  'pesquisa' returna a entrada mais recente para uma
-- dada chave.  Isso é feito por questão de eficiência. Usar
-- 'atualiza' para obter uma única entrada para cada val.
...
-- -----------------------------------------------------------------------------
atualiza :: (Eq chave, Hashable chave)=>TabDisp chave val->chave->val->IO ()
atualiza tab chave val =
  atualizaTab (\x -> let x' = remAll (==chave) x in ((chave,val):x')) tab chave
