...
O uso de arranjos é uma maneira alternativa de representar listas em
uma linguagem (como \C) que não provê suporte a manipulação de valores
...
\or\ (\or\ usa \foldr), como a seguir (\mmap\ e \foldr\ são ferramentas
...
Na versão imperativa, incluímos uma versão de listas duplamente
encadeadas (Seção \ref{lista-duplamente-encadeada}, onde reescrevemos
as funções \elem\ e \insert) para realizar a procura com chamada à
função \elem\ seguida de chamada a \insert.
...
\progb{\delete\ ($a$, $l$) \{ \\
    ...
    \hspace*{.2cm} \iif ($p$ != \NULL)\\
       \hspace*{.5cm} \iif (\prev\ == \NULL)\\ 
	  \hspace*{1cm} \return\ $l$->$r$;
       \hspace*{.5cm} \eelse\ \{ \prev->$r$ = $p$->$r$; \return\ $l$; \}
  }
...
newtype \Pilha\ $a$ = \Pilha\ [$a$]\\ \ \hspace*{.2cm} \\ 
