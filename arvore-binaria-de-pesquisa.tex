...
Para evitar o cálculo da altura de sub-árvores direita e esquerda para
determinar o fator de balanceamento de cada nodo que está no caminho
da árvore até o nodo inserido, que seria desnecessariamente
ineficiente, e para facilitar a atualização da altura, vamos armazenar
e atualizar as alturas de ambas as sub-árvores de cada nodo.
...
type AlturaEsq = Integer
type AlturaDir = Integer

data ArvoreAVL a = Vazia | Nodo a (AlturaEsq,AlturaDir) (ArvoreAVL a) (ArvoreAVL a)

arvVazia = Vazia

ins:: Ord a => a -> ArvoreAVL a -> ArvoreAVL a
ins k Vazia             = Nodo k (0,0) Vazia Vazia
ins k arv@(Nodo v (ae,ad) e d) = 
 case compare k v of 
  LT -> let e1@(Nodo v1 (ae1,ad1) _ _) = ins k e 
            altura1 = max ae1 ad1
            altura' = altura1+1
         in if altura1 - ad == 1 -- condição AVL precisa ser restaurada (altura' = altura1 + 1)
            then if k < v1 
                 then sobE  (Nodo v (altura',ad) e1 d) 
                 else sobED (Nodo v (altura',ad) e1 d) 
            else Nodo v (altura',ad) e1 d
  GT -> let d1@(Nodo v1 (ae1,ad1) _ _) = ins k d 
            altura1 = max ae1 ad1
            altura' = altura1+1
         in if altura1 - ae == 1 -- condição AVL precisa ser restaurada (altura' = altura1 + 1)
            then -- ou sobD ou sobDE 
               if k > v1
               then sobD  (Nodo v (ae,altura') e d1) 
               else sobDE (Nodo v (ae,altura') e d1) 
            else Nodo v (ae,altura') e d1
  _ -> arv

sobE :: ArvoreAVL a -> ArvoreAVL a
sobE (Nodo v (ae,ad) (Nodo ve (ae1,ad1) ee ed) d) = Nodo ve (ae1,max ad1 ad +1) ee (Nodo v (ad1,ad) ed d) 

sobD :: ArvoreAVL a -> ArvoreAVL a
sobD (Nodo v (ae,ad) e (Nodo vd (ae1,ad1) de dd)) = Nodo vd (max ae ae1+1,ad1) (Nodo v (ae,ae1) e de) dd
  
sobED :: ArvoreAVL a -> ArvoreAVL a
sobED (Nodo v (ae,ad) (Nodo ve (aee,aed) ee (Nodo ved (aede,aedd) ede edd)) d) = 
       Nodo ved (max aee aede+1,max aedd ad+1) (Nodo ve (aee,aede) ee ede) (Nodo v (aedd,ad) edd d)

sobDE :: ArvoreAVL a -> ArvoreAVL a
sobDE (Nodo v (ae,ad) e (Nodo vd (ade,add) (Nodo vde (adee,aded) dee ded) dd)) = 
       Nodo vde (max ae adee+1,max aded add+1) (Nodo v (ae,adee) e dee) (Nodo vd (aded,add) ded dd)
\end{hask}
\end{tabular}
\end{center}
