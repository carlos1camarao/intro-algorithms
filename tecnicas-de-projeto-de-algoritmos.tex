...
\item[Decremento] (em inglês, chamada de {\em
  decrease-and-conquer\/}): é baseada em passar da instância original
  para uma instância menor ou reduzir o tamanho da entrada do problema
  para solução da instância original. O decréscimo é tipicamente de um
  valor ou por um fator constante no tamanho da entrada, mas pode
  também ser um decréscimo de um valor ou por um fator variável. Note
  que o decremento por um fator constante não é caracterizado como
  divisão (seguindo \cite{Levitin:Introd-Design-Analysis-Alg}): a
...
celebridade :: NumPessoas -> [Pessoa] -> (ConheceM,[(Pessoa,Pessoa)]) -> Pessoa
-- Pre: celebridade n x m implica n = length x
celebridade n (a:b:r) mx@(m,_)           
  | findWithDefault False (a,b) m = celebridade (n-1) (b:r) mx -- 'a' não é celeb. (celeb. não conhece ninguém)
  | otherwise                     = celebridade (n-1) (a:r) mx -- 'b' não é celeb. (todos conhecem celeb.)
celebridade _ [a]     (m,x)       = null (map (==a) . fst) x   -- se existe 'b': (a,b) está em 'x', 'a' não é celeb.
