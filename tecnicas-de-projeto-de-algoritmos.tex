...
\item[Decremento] (em inglês, chamada de {\em decrease-and-conquer\/}):
  ...
  A maioria dos algoritmos apresentados neste livro usam a técnica de
  decremento: veja por exemplo \inh{maiorElem} (seção
  \ref{maior-elemeento}), \inh{elem} (seção
  ...
  \inh{delete} (seção \ref{sec:insertion-sort-func}), multiplicação
  de matrizes (seção \ref{multiplicacao-de-matrizes}), função
  \inh{fib} (seção \ref{numeros-de-fibonacci}).
...
domínio de \inh{n} (ou seja, o par \inh{(a,b)} não foi inserido como
