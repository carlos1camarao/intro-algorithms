...
Pesquisar em computação significa encontrar um dado valor dentre
vários valores existentes. Os valores existentes podem estar
representados como um conjunto ou qualquer forma de agrupamento de
valores. 

-- Existem vários algoritmos de pesquisa. Na seção
-- \ref{pesquisa-em-lista} apresentamos um algoritmo simples de {\em
--   pesquisa sequencial}. Na seção --
-- \ref{pesquisa-sequencial-com-valores-ordenados} apresentamos pequena
-- variação da pesquisa sequencial usada quando os valores que estão
-- sendo pesquisados estão ordenados. Na seção
-- \ref{pesquisa-sequencial-com-sentinela} apresentamos % outra pequena
-- variação (otimização) da pesquisa sequencial, que usa o que é chamado
-- de {\em sentinela}. Sentinela é um elemento adicionado a extremo de
-- estrutura de dados para evitar teste para verificar chegada a esse
-- extremo.

A seção \ref{pesquisa-binaria} apresentamos o eficiente algoritmo de
{\em pesquisa binária\/}. 
