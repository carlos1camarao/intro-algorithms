...
\begin{quotation}
  {\em Uma relação de recorrência é uma definição recursiva para a
    qual busca-se (espera-se encontrar) uma solução não recursiva que
    a simplifique, que especifica a mesma relação (em geral uma
    função).\/}
\end{quotation}

Note: essa busca pode não ter sucesso. De fato, a grande maioria das
equações recursivas não pode ser {\em simplificada\/} para uma solução
não recursiva, embora possa ser reescrita, algumas vezes com bastante
esforço, para uma forma não diretamente recursiva, mas que use alguma
maneira (estrutura de dados) de implementar (para representar) a
recursão. No entanto, as equações que expressam tempos de execução de
pequenos programas, com as quais vamos trabalhar, em geral são
bastante simples e podem ser {\em simplificadas\/} para uma equação
não recursiva.

No caso de um programa não recursivo, a
expressão-determinante-da-eficiência é em geral um somatório, que em
geral também pode ser simplificado, usando propriedades de somatórios
como as seguintes, onde $a,b,c$ são constantes:
...
\begin{center}
\begin{tabular}{l}
\begin{hask}{allUnique,chkUnique}
