...
expressão-determinante-da-eficiência é em geral uma {\em relação de
  recorrência}. Uma relação de recorrência é uma definição recursiva
para a qual em geral existe uma solução não recursiva que a
simplifique (especificando a mesma relação). No caso de um programa
não recursivo, a expressão-determinante-da-eficiência é em geral um
somatório, que em geral também pode ser simplificado.
...
A função \foldl'\ se comporta de modo similar a \foldl, mas ``menos
preguiçosa'': \foldl' $f$ força a avaliação de $f$, de modo que, {\tt
  $z$ `$f$` $e_1$} seja avaliado antes de compor a expressão ($z$ `$f$`
$e_1$) `$f$` $e_2$), e assim sucessivamente. Isso não altera a
complexidade assintótica do tempo de execução, mas pode afetar
bastante a eficiência o espaço necessário para execução......

A complexidade é a mesma da pesquisa sequencial em uma lista: $O(n)$,
no pior caso. ...... pois envolve possivelmente comparação com cada
elemento da lista ...
