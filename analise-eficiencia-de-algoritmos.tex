...
A função \inh{verifUnico} usa um par $(b,x)$, onde $b$ indica
unicidade (de todos os elementos testados até agora), e $x$ representa
a lista dos elementos já testados. A unicidade de cada elemento $a$ da
lista é testada com relação à lista dos já testados $x$: se $a$ não
pertence a $x$, então a unicidade é preservada e $a$ é adicionado a
$x$, caso contrário não há unicidade e a verificação pode ser
interrompida.

A função \inh{verifUnicidade} percorre a lista para produzir o
...
Mostre que as duas definições têm complexidade de tempo $O(lg n)$.
...
existir, caso contrário indicar que não existe.

Escreva um programa que leia os dados de entrada --- fornecidos em
formato adequado, que você deve especificar --- e resolva o problema
de determinar a celebridade do grupo de pessoas, se existir, e caso
contrário indicar que não existe.
