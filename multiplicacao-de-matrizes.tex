...
\label{multiplicacao-de-matrizes}
...
\label{versao-funcional-de-matrixMult}
...
\begin{center}
\begin{tabular}{l}
\begin{hask}{fatorial,fibs}
fatorial n = fatArr 
  where fatArr = array (k,n) ((1,1) : [(i, i * fatArr!(i-1)) | i <- [2..n]]) 

fibs n  = fibArr 
  where fibArr = array (0,n) ([(0, 1), (1, 1)] ++ 
                              [(i, fibArr!(i-2) + fibArr!(i-1)) | i <- [2..n]]) 
\end{hask}
\end{tabular}
\end{center}
...
É incluída função \inh{transpose} para transposição de matrizes
...
\inh{matrixMult} e considerando a segunda matriz (\inh{m2}) como uma
matriz de linhas, deve-se usar: \inh{matrixMult m1 (transpose m2)}.
...
\begin{center}
\begin{tabular}{l}
\begin{hask} {matrixMult,transpose}
...
type MatrixLin = Array Integer Lin -- MatrixLin = arranjo de linhas 
type MatrixCol = Array Integer Col -- MatrixCol = arranjo de colunas
type Lin       = Array Integer Integer
type Col       = Array Integer Integer
type LinNum    = Integer
type ColNum    = Integer
  
matrixMult :: MatrixLin -> MatrixCol -> MatrixLin
...
addLin :: MatrixLin -> MatrixCol -> ColNum -> ColNum -> MatrixLin -> LinNum -> MatrixLin
-- Adiciona m!i. Cada elemento j de m!i = produto interno de m1!i pela coluna j de m2.
addLin m1 m2 numColsm1 numCols m i = m // [(i,mi)]
  where mi = mkLin i (m1!i) m2 numColsm1 numCols 

mkLin:: LinNum -> Lin -> MatrixCol -> ColNum -> ColNum -> Lin
-- m!i = linha formada pelo produtos internos de m1!i por m2!j, para j = [1..numCols]
mkLin i m1i m2 numColsm1 numCols = array (1,numCols) [(j, innerProd m1i j (m2!j) numColsm1) | j <- [1..numCols]]
      
innerProd :: Lin -> ColNum -> Col -> ColNum -> Integer
innerProd m1i j m2j n = sum [(m1i!j) * (m2j!j) | j <- [1..n]]

transpose :: MatrixLin -> MatrixCol
...
transpose m = array (1,numCols) [(i, array (1,numLins) [(j, m!j!i) | j <- [1..numLins]]) | i <- [1..numCols]]
  where
    (_,numLins) = bounds m
...
\begin{center}
\begin{tabular}{l}
\begin{alg}{matrixMult,mkSquareMatrix}

int** mkSquareMatrix (int n) {
  int i,j; 
